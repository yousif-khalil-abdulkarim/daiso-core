...
:::info
Note If the same owner acquires the lock multiple times, the subsequent calls will succeed without error.
By default each created lock will have unique owner.

```ts
// First acquisition by the owner
await lock.aquireOrFail();

// Subsequent acquisitions by the same owner will not throw an error
await lock.aquireOrFail();
```
