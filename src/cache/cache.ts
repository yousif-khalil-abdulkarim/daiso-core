...
export class Cache<TType> implements ICache<TType> {
    ...
    private withDefaultTTLValue<TValue>(
    ...
    async addMany<TValues extends TType, TKeys extends string>(
        ...
        try {
            return await this.cacheAdapter.addMany(
                this.withDefaultTTLValue(values),
            );
        } catch (error: unknown) {
            if (error instanceof CacheError) {
                throw error;
    ...
    async putMany<TValues extends TType, TKeys extends string>(
        ...
        try {
            return await this.cacheAdapter.putMany(
                this.withDefaultTTLValue(values),
            );
        } catch (error: unknown) {
            if (error instanceof CacheError) {
        ...
        try {
            if (typeof valueToAdd !== "function") {
                return await this.cacheAdapter.getOrAdd(
                    key,
                    valueToAdd as TExtended,
                    ttlInMs,
                );
            }
            const value = await this.get(key);
            if (value === null) {
                const simpleValueToAdd = (await simplifyAsyncLazyable(
                    valueToAdd,
                )) as TExtended;
                await this.add(
                    key,
                    simpleValueToAdd as CacheValue<TExtended>,
                    ttlInMs,
                );
