...
import { ICache } from "@/contracts/cache/cache.contract";
import {
    type TestAPI,
    type SuiteAPI,
    type beforeEach,
...
export function cacheApiTestSuite({
    test,
    expect,
    describe,
    beforeEach,
    ...
    describe("Api tests", () => {
        ...
        describe("method: getOr", () => {
            ...
            test("Should return default value when given none existing key", async () => {
                expect(await cache.getOr("a", -1)).toBe(-1);
            });
        });
        ...
        describe("method: add", () => {
            test("Should throw CacheError when given NaN as input", async () => {
                await expect(cache.add("a", NaN)).rejects.toBeInstanceOf(
                    CacheError,
                );
            });
            test("Should throw InvalidValueCacheError when given NaN as input", async () => {
                await expect(cache.add("a", NaN)).rejects.toBeInstanceOf(
                    InvalidValueCacheError,
                );
            });
        ...
        describe("method: addMany", () => {
            ...
            test("Should throw InvalidValueCacheError when given NaN as input", async () => {
                ...
                ).rejects.toBeInstanceOf(InvalidValueCacheError);
            });
        ...
        describe("method: put", () => {
            test("Should throw CacheError when given NaN as input", async () => {
                await expect(cache.put("a", NaN)).rejects.toBeInstanceOf(
                    CacheError,
                );
            });
            test("Should throw InvalidValueCacheError when given NaN as input", async () => {
        ...
        describe("method: getAndRemove", () => {
            ...
            test("Should remove key when key exists", async () => {
                await cache.add("a", 1);
        ...
        describe("method: getOrAdd", () => {
            test("Should throw CacheError when given NaN as input", async () => {
                await expect(cache.getOrAdd("a", NaN)).rejects.toBeInstanceOf(
                    CacheError,
                );
            });
            test("Should throw InvalidValueCacheError when given NaN as input", async () => {
            ...
            test("Should return insertion value when key doesnt exists", async () => {
                await cache.getOrAdd("a", -1);
                expect(await cache.get("a")).toBe(-1);
            });
            test("Should return insert value when key doesnt exists", async () => {
                expect(await cache.getOrAdd("a", -1)).toBe(-1);
            });
        ...
        describe("method: increment", () => {
            ...
            test("Should set value to 0 if key doesn't exists", async () => {
                await cache.increment("a", 1);
