...
import { AsyncPipeline } from "@/utilities/classes/pipeline/async-pipeline.js";

describe("class: AsyncPipeline", () => {
    test("Should call original function", async () => {
        function fn(nbr: number): number {
            return nbr + 1;
        }
        const value = await new AsyncPipeline(fn, []).invoke(1);
        expect(value).toBe(2);
    });
    ...
    test("Should change return value from first middleware", async () => {
        ...
        const result = await new AsyncPipeline(fn, [
            async (args, next) => {
