    ...
    implements ILockAdapter, IDeinitizable, IInitizable
{
    ...
    async acquire(
        ...
        // Cannot be null when upserting and returnDocument is set to "after"
        if (lockData === null) {
            throw new UnexpectedError(`Expected "lockData" to be not null.`);
        }

        if (lockData.expiration === null) {
            const isCurrentOwner = lockData.owner === owner;
            return isCurrentOwner;
        }

        const isCurrentOwner = lockData.owner === owner;
        const isNotExpired = lockData.expiration > new Date();
        return isCurrentOwner && isNotExpired;
    }
    ...
    async forceRelease(key: string): Promise<void> {
        ...
        if (!result.acknowledged) {
            throw new UnexpectedError("Deletion was not acknowledged");
        }
    }
