...
export function databaseLockAdapterTestSuite(
    ...
    describe("method: updateIfExpired", () => {
        test("Should return null after update when key doesnt exists", async () => {
            const noneExistingKey = "key";
            const noneExistingOwner = "b";
            const expiration = TimeSpan.fromMinutes(4).toStartDate();

            const lockData = await adapter.updateIfExpired(
                noneExistingKey,
                noneExistingOwner,
                expiration,
            );
        ...
        test("Should update when key is expired", async () => {
            const key = "key";
            const ownerA = "a";
        ...
        test("Should not update when key is unexpired", async () => {
            const key = "key";
            const ownerA = "a";
        ...
        test("Should not update when key is unexpirable", async () => {
            const key = "key";
            const ownerA = "a";
