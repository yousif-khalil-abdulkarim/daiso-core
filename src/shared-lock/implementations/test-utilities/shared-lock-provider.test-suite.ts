...
export type SharedLockProviderTestSuiteSettings = {
    ...
    createSharedLockProvider: () => Promisable<ISharedLockProvider>;
    serde?: ISerde;
...
/**
 * The `lockProviderTestSuite` function simplifies the process of testing your custom implementation of {@link ISharedLock | `ISharedLock`} with `vitest`.
 *
 * IMPORT_PATH: `"@daiso-tech/core/shared-lock/test-utilities"`
 * @group Utilities
 * @example
 ...
 * import { lockProviderTestSuite } from "@daiso-tech/core/shared-lock/test-utilities";
 ...
 *     let map: Map<string, ISharedLockData>;
...
export function sharedLockProviderTestSuite(
    ...
    let lockProvider: ISharedLockProvider;
    beforeEach(async () => {
        lockProvider = await createSharedLockProvider();
    });
