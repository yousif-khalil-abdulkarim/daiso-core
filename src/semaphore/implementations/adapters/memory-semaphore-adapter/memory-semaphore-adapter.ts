...
export class MemorySemaphoreAdapter implements ISemaphoreAdapter {
    ...
    async getState(key: string): Promise<ISemaphoreState | null> {
        const semaphore = this.map.get(key);
        if (semaphore === undefined) {
            return null;
        }
        return {
            limit: semaphore.slots.size,
    ...
    async acquire(settings: SemaphoreAcquireSettings): Promise<boolean> {
        ...
        if (expiration === null) {
            semaphore.slots.set(slotId, { timeoutId: null, expiration });
        } else {
            const timeoutId = setTimeout(() => {
                semaphore.slots.delete(slotId);
            }, expiration.getTime());

            semaphore.slots.set(slotId, { timeoutId, expiration });
        }
