    ...
    describe("Api tests:", () => {
        ...
        describe("method: acquire", () => {
            ...
            test("Should not update limit when slot count is more than 0", async () => {
                ...
                const result2 = await semaphore3.getLimit();
                expect(result2).toBe(limit);
            });
        });
        ...
        describe("method: acquireBlocking", () => {
            ...
            test("Should not update limit when slot count is more than 0", async () => {
                ...
                const result = await semaphore3.getLimit();
        ...
        describe("method: release", () => {
            ...
            test("Should update limit when slot count is 0", async () => {
                ...
                const result2 = await semaphore5.getLimit();
                expect(result2).toBe(newLimit);
        ...
        describe("method: releaseOrFail", () => {
            test("Should throw UnsuccessfulReleaseSemaphoreError when key doesnt exists", async () => {
                const key = "a";
                const limit = 2;
                const ttl = null;
                await semaphoreProvider
            ...
            test("Should throw UnsuccessfulReleaseSemaphoreError when slot is expired", async () => {
                const key = "a";
                const ttl = TimeSpan.fromMilliseconds(50);
                const limit = 2;
            ...
            test("Should throw UnsuccessfulReleaseSemaphoreError when slot exists, is expired", async () => {
                const key = "a";
                const ttl = TimeSpan.fromMilliseconds(50);
                const limit = 2;
        ...
        describe("method: refreshOrFail", () => {
            test("Should throw UnsuccessfulRefreshSemaphoreError when key doesnt exists", async () => {
                const key = "a";
                const limit = 2;
                const ttl = null;
                const semaphore1 = semaphoreProvider.create(key, {
            ...
            test("Should throw UnsuccessfulRefreshSemaphoreError when slot is expired", async () => {
                const key = "a";
                const limit = 2;
                const ttl = TimeSpan.fromMilliseconds(50);
                const semaphore = semaphoreProvider.create(key, {
            ...
            test("Should throw UnsuccessfulRefreshSemaphoreError when slot exists, is expired", async () => {
                const key = "a";
                const ttl = TimeSpan.fromMilliseconds(50);
                const limit = 2;
            ...
            test("Should throw UnsuccessfulRefreshSemaphoreError when slot exists, is unexpireable", async () => {
                const key = "a";
                const ttl = null;
                const limit = 2;
        ...
        describe("method: isExpired", () => {
            test("Should return false when is unexpireable", async () => {
                ...
                await semaphore.acquire();
                const result = await semaphore.isExpired();
            ...
            test("Should return true when limit is reached", async () => {
                ...
                const result = await semaphore2.isExpired();
        ...
        describe("method: isAcquired", () => {
            test("Should return true when is unexpireable", async () => {
                ...
                await semaphore.acquire();
                const result = await semaphore.isAcquired();
            ...
            test("Should return false when limit is reached", async () => {
                ...
                const result = await semaphore2.isAcquired();

                expect(result).toBe(false);
            });
        });
        describe("method: getLimit", () => {
            test("Should return limit when key doesnt exists", async () => {
                const key = "a";
                const limit = 2;
                const ttl = null;
                const semaphore = semaphoreProvider.create(key, {
                ...
                const result = await semaphore.getLimit();
        ...
        describe("method: availableSlots", () => {
            test.todo("Write tests!!!");
        });
        describe("method: unavailableSlots", () => {
            test.todo("Write tests!!!");
        });
    });
